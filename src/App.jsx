...
// Calcular distancia entre dos puntos (Haversine) para evitar refrescos innecesarios
...
function ContenedorPedidos() {
  ...
  const ultimaPosicionRef = useRef(null); // Evita parpadeo
  ...
  const iniciarNavegacion = (clienteId) => {
    ...
    watchId.current = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        // Solo actualizamos el mapa si el repartidor se movió más de 15 metros
        if (!ultimaPosicionRef.current || calcularDistancia(ultimaPosicionRef.current.lat, ultimaPosicionRef.current.lng, latitude, longitude) > 15) {
          ultimaPosicionRef.current = { lat: latitude, lng: longitude };
          setPosicionActual({ lat: latitude, lng: longitude });
        }
      },
      (err) => alert("Activa el GPS y da permisos de ubicación."),
  ...
  if (cargando) return <div className="h-screen w-full flex items-center justify-center font-black text-indigo-600 animate-pulse text-2xl uppercase">Sincronizando Despachos...</div>;

  // --- MODO PANTALLA COMPLETA (GOOGLE MAPS) ---
  if (mapaFullscreen && posicionActual) {
    const clienteActivo = clientesAgrupados.find(c => c.idUnico === verMapa);
    // q=coordenadas (pin usuario) | daddr=destino | ll=CENTRO DE CÁMARA | z=19 (zoom calle)
    const urlGoogle = `https://maps.google.com/maps?q=${posicionActual.lat},${posicionActual.lng}&saddr=${posicionActual.lat},${posicionActual.lng}&daddr=${encodeURIComponent(clienteActivo.direccion)}&ll=${posicionActual.lat},${posicionActual.lng}&z=19&t=m&output=embed&iwloc=near`;
    ...
    return (
      <div className="fixed inset-0 z-[500] bg-black flex flex-col overflow-hidden animate-in fade-in">
        <div className="absolute top-6 left-4 right-4 z-[510] flex gap-2">
          <button onClick={detenerNavegacion} className="bg-white/95 p-4 rounded-2xl shadow-2xl text-red-500 active:scale-90 transition border border-slate-100"><XCircle size={26} /></button>
        ...
        {/* Usamos el src dinámico. El navegador no hará flash blanco si la URL cambia suavemente sin 'key' */}
        <iframe className="w-full h-full border-none" frameBorder="0" src={urlGoogle} allowFullScreen></iframe>

        <div className="absolute bottom-8 left-8 right-8 z-[510]">
          <button onClick={() => setMapaFullscreen(false)} className="w-full bg-slate-900/90 text-white py-5 rounded-[2rem] font-black uppercase text-xs shadow-2xl flex items-center justify-center gap-3 border border-white/10">
              ...
              <div key={cliente.idUnico} className={`w-full bg-white rounded-[2.5rem] shadow-xl border-4 transition-all ${estaAbierto ? 'border-indigo-500' : 'border-transparent'}`}>
                <div className="p-6 md:p-10 cursor-pointer border-l-[16px] border-slate-900" onClick={() => { setExpandido(estaAbierto ? null : cliente.idUnico); detenerNavegacion(); }}>
                   ...
                   <div className="flex justify-between items-end pt-6 border-t border-slate-100">
                    <div className="flex-1"><p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{estaAbierto ? 'Recaudación Carga' : 'Pendientes'}</p><p className="text-4xl font-black text-slate-800">{estaAbierto ? `$${cliente.totalGeneral.toLocaleString('es-CL')}` : cliente.pedidos.length + ' Pedidos'}</p></div>
                ...
                {estaAbierto && (
                  <div className="bg-slate-50 p-4 md:p-10 border-t-4 border-slate-100 w-full animate-in fade-in">
                    ...
                    <div className="flex flex-col gap-6 w-full">
                      <button onClick={() => iniciarNavegacion(cliente.idUnico)} className="bg-slate-800 text-white py-6 rounded-[2rem] font-black flex items-center justify-center gap-4 shadow-2xl uppercase tracking-widest text-lg hover:bg-slate-900 transition-all"><Navigation2 size={28}/> Iniciar GPS (Centrado en mí)</button>
