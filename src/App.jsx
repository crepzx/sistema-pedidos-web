...
function ContenedorPedidos() {
  ...
  async function confirmarTodoElCliente(pedidosCliente) {
    ...
    if (window.confirm(`¿Confirmar entrega de los ${ids.length} documentos de este cliente?`)) {
  ...
  if (cargando) return <div className="text-center p-20 font-black">Sincronizando Hoja de Ruta...</div>;

  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-10 font-sans text-slate-900">
      <div className="max-w-5xl mx-auto"> 
        <header className="mb-8 border-b-2 border-slate-200 pb-6 flex justify-between items-center">
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-slate-800 flex items-center gap-2">
              <Package className="text-blue-600" size={32} /> Despachos Pendientes
            </h1>
            ...
            return (
              <div key={cliente.idUnico} className={`w-full bg-white rounded-[2rem] shadow-xl border-2 transition-all overflow-hidden ${estaAbierto ? 'border-blue-500' : 'border-transparent'}`}>
                ...
                >
                  <div className="flex flex-wrap justify-between items-center mb-4 gap-3">
                    <div className="flex gap-2 items-center">
                      <span className={`text-[10px] font-black px-3 py-1 rounded-full text-white uppercase tracking-tighter shadow-sm ${esFactura ? 'bg-orange-500' : 'bg-blue-500'}`}>
                        <FileText size={12} className="inline mr-1"/> {esFactura ? 'Factura' : 'Boleta'}
                      </span>
                  ...
                  <h2 className="text-2xl font-black text-slate-800 mb-2 uppercase italic leading-tight">{cliente.nombre}</h2>
                  ...
                  <div className="flex justify-between items-end pt-4 border-t border-slate-100">
                    <div>
                      {estaAbierto ? (
                        <>
                          <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Total General a Cobrar</p>
                        ...
                        <>
                          ...
                          <p className="text-2xl font-black text-slate-700">{cliente.pedidos.length} Documento(s)</p>
                        </>
                      )}
                    </div>
                    <div className="flex flex-col items-center text-blue-600 font-black text-[10px] uppercase">
                      {estaAbierto ? <ChevronUp size={28}/> : <ChevronDown size={28}/>}
                    </div>
                  </div>
                </div>

                {/* DETALLE EXPANDIDO POR CADA PEDIDO */}
                {estaAbierto && (
                  <div className="bg-slate-50 p-4 md:p-8 border-t-2 border-slate-100 animate-in fade-in duration-300">
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                      {cliente.pedidos.map((p) => (
                        <div key={p.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col transition-all hover:shadow-md">
                          <div className="p-3 bg-slate-800 text-white flex flex-wrap justify-between items-center gap-2">
                            <span className="text-xs font-black tracking-widest uppercase">FOLIO #{p.folio}</span>
                            <div className="flex gap-3">
                                <span className="text-[9px] font-bold opacity-80 flex items-center gap-1"><Calendar size={12}/> {p.fecha_entrega}</span>
                          ...
                          <div className="p-4 flex-1">
                            ...
                            {/* Muestra quien recibe solo si el campo tiene texto */}
                            {p.quien_recibe && p.quien_recibe.trim().length > 0 && (
                                <div className="mt-3 flex items-center gap-2 text-blue-600 bg-blue-50 p-2 rounded-lg">
                                    <UserCheck size={14} />
                                    <p className="text-[10px] font-black uppercase">Recibe: {p.quien_recibe}</p>
                                </div>
                            )}
                          </div>
                          <div className="p-4 bg-slate-50 border-t flex justify-between items-center">
                            <span className="text-lg font-black text-emerald-600">${Number(p.total_pedido).toLocaleString('es-CL')}</span>
                            <button onClick={() => confirmarEntregaFolio(p.id, p.folio)} className="bg-emerald-500 text-white p-2 rounded-xl hover:bg-emerald-600 shadow-md transition active:scale-90">
                              <CheckCircle size={20}/>
                            </button>
                          </div>
                        </div>
                    ...
                    <div className="flex flex-col gap-3 max-w-2xl mx-auto">
                      <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(cliente.direccion)}`} target="_blank" rel="noreferrer" className="bg-slate-800 text-white py-4 rounded-2xl font-black flex items-center justify-center gap-3 shadow-xl hover:bg-slate-900 transition uppercase tracking-widest text-sm">
                        <Navigation size={22}/> Navegar hacia {cliente.nombre}
                      </a>
                      
                      <button onClick={() => confirmarTodoElCliente(cliente.pedidos)} className="bg-emerald-600 text-white py-4 rounded-2xl font-black flex items-center justify-center gap-3 shadow-xl hover:bg-emerald-700 transition uppercase tracking-widest text-sm">
                        <CheckCircle size={22}/> Confirmar Entrega Total
                      </button>

                      <button onClick={() => setExpandido(null)} className="bg-slate-200 text-slate-600 py-3 rounded-2xl font-black flex items-center justify-center gap-3 hover:bg-slate-300 transition uppercase tracking-widest text-xs">
