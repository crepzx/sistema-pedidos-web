...
// --- ICONO PERSONALIZADO PARA EL REPARTIDOR (Punto Azul) ---
const iconoRepartidor = new L.Icon({
    iconUrl: 'https://cdn-icons-png.flaticon.com/512/717/717681.png',
    iconSize: [40, 40],
    iconAnchor: [20, 20]
});
...
function CentrarMapa({ posicion }) {
  ...
  useEffect(() => {
    if (posicion) {
      map.setView([posicion.lat, posicion.lng], 18, { animate: true });
    }
  }, [posicion, map]);
  return null;
}
...
function ContenedorPedidos() {
  ...
  // --- MODO PANTALLA COMPLETA (LEAFLET - SIN PESTAÑEO) ---
  if (mapaFullscreen && posicionActual) {
    ...
    return (
      <div className="fixed inset-0 z-[500] bg-white flex flex-col overflow-hidden animate-in fade-in">
        ...
        {/* MAPA DE LEAFLET (Sustituye al Iframe de Google) */}
        <MapContainer 
        ...
        >
          ...
          <CentrarMapa posicion={posicionActual} />
        </MapContainer>

        <div className="absolute bottom-6 left-6 right-6 z-[510]">
          <button onClick={() => setMapaFullscreen(false)} className="w-full bg-slate-900 text-white py-5 rounded-[2rem] font-black uppercase text-[10px] shadow-2xl flex items-center justify-center gap-3 active:scale-95 transition">
        ...
        <div className="w-full max-w-sm bg-white rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col max-h-[98vh]">
          ...
          <div className="p-6 flex-1 flex flex-col justify-between">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={() => { setFotoPreview(null); setTimeout(() => fileInputRef.current.click(), 150); }} className="w-full bg-slate-100 text-indigo-700 py-3 rounded-2xl font-black text-xs uppercase">Repetir Foto</button>
                ...
                <div className="p-6 md:p-10 cursor-pointer border-l-[16px] border-slate-900" onClick={() => { setExpandido(estaAbierto ? null : cliente.idUnico); detenerNavegacion(); }}>
                   ...
                   <div className="flex justify-between items-end pt-6 border-t border-slate-100">
                    <div className="flex-1">
                      <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{estaAbierto ? 'Total Acumulado' : 'Próxima Entrega'}</p>
                      ...
                      {cliente.pedidos.map((p) => {
                        ...
                        return (
                          <div key={p.id} className="bg-white rounded-[2rem] shadow-sm border-2 border-slate-200 overflow-hidden flex flex-col">
                            ...
                            <div className="p-6 bg-slate-50 border-t flex justify-between items-center"><span className="text-2xl font-black text-emerald-600">${Number(p.total_pedido).toLocaleString('es-CL')}</span><button onClick={() => iniciarCaptura(p)} className="bg-emerald-500 text-white p-3 rounded-2xl shadow-lg hover:scale-105 active:scale-90 transition"><Camera size={24}/></button></div>
                          </div>
                        );
                      })}
                    </div>
                    <div className="flex flex-col gap-6 w-full">
                      <button onClick={() => iniciarNavegacion(cliente.idUnico)} className="bg-slate-800 text-white py-6 rounded-[2rem] font-black flex items-center justify-center gap-4 shadow-2xl uppercase tracking-widest text-lg hover:bg-slate-900 transition-all"><Navigation2 size={28}/> Iniciar GPS (Sigue mi ruta)</button>
